...
namespace GamingBooster_Pro
{
    ...
    public sealed class RedlineAppData
    {
        ...
        /// <summary>V9 Launch: Free only. Pro license comes in a later update.</summary>
        ...
        public string ProSourceLabel
        {
            get
            {
                if (ProLicenseActive || Tier == LicenseTier.Pro)
                    return Language == "EN" ? "License key" : "Lizenzschlüssel";
                if (DevProEnabled)
                    return Language == "EN" ? "Developer test" : "Entwickler-Test";
                return Language == "EN" ? "Free" : "Free";
            }
        }

        /// <summary>Gültige Pro-Schlüssel für V9 (lokal, offline).</summary>
        ...
        public bool TryActivateLicenseKey(string input, out string error)
        {
            ...
            bool ok = ValidProKeys.Any(k => string.Equals(k, key, StringComparison.OrdinalIgnoreCase));
            if (!ok)
            {
                error = Language == "EN"
                    ? "Invalid license key."
